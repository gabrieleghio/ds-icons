...
**Current state:** 11 icons across 6 categories, 6 sizes per icon (16, 20, 24, 32, 40, 48). Scalable to hundreds of icons.
...
- **6 sizes per icon** (16, 20, 24, 32, 40, 48) — all pixel-perfect
...
The `raw/` directory contains SVG files, one for each icon×size:
...
│   ├── Close_16.svg
│   └── ...
└── ApplePay/      ← raster icons (SVG with embedded PNG)
    ├── ApplePay_16.svg
    └── ...
```
...
export const Home32 = ({ color = 'currentColor', className, style }) => (
  <svg viewBox="0 0 32 32" width={32} height={32} fill={color}
...
export const renderHome32 = (color = 'currentColor', className?: string) =>
  html`<svg viewBox="0 0 32 32" width="32" height="32" fill="${color}"
...
.button { color: var(--color-primary); }
.button:hover { color: var(--color-primary-dark); }
.icon { /* has no effect on SVG content */ }
```
...
```
raw/{IconName}/{IconName}_{size}.svg
    ↓ (SVGR for React)
packages/react/src/icons/{IconName}{size}.tsx
    ↓ (Rollup + preserveModules)
dist/icons/Home32.js
    ↓ (barrel index)
dist/index.js  ← @myorg/icons-react
    
raw/{IconName}/{IconName}_{size}.svg
    ↓ (custom script for Lit)
packages/lit/src/icons/render{IconName}{size}.ts
    ↓ (Rollup + preserveModules)
dist/icons/renderHome32.js
    ↓ (barrel index)
...
```bash
raw/[category]/[IconName]/[IconName]_[size].svg → packages/react/src/[category]/[IconName][size].tsx
```
...
```bash
raw/[category]/[IconName]/[IconName]_[size].svg → packages/lit/src/[category]/render[IconName][size].ts
```
...
│   │   ├── Home32.tsx           {generated}
│   │   ├── Home24.tsx           {generated}
│   │   ├── Backpack24.tsx       {generated}
│   │   └── ...
│   ├── index.ts                 {generated}
